...
> 好的面试官。Reactor和Proactor模式在技术实现和职责划分上有清晰的区别：
>
> 1.  **Reactor（反应器模式）**：
>     * **核心**：基于**I/O就绪**通知。
>     * **流程**：应用程序将文件描述符（Handle）和回调处理器（Handler）注册到Reactor中。Reactor通过事件多路复用机制（如`epoll_wait`）等待事件。当某个文件描述符就绪时，Reactor被唤醒，并调用对应的Handler。
>     * **I/O执行者**：**应用程序线程**。Handler在被回调时，需要自己执行`read`、`write`、`accept`等同步非阻塞的I/O操作。
>     * **优点**：模型相对简单，跨平台性好，因为`select`/`poll`/`epoll`在各类操作系统上支持良好，是目前主流框架（如Netty, Redis）的选择。
>
> 2.  **Proactor（前摄器模式）**：
>     * **核心**：基于**I/O完成**通知。
>     * **流程**：应用程序发起一个**异步I/O操作**，并提供一个缓冲区和完成回调（Completion Handler）。然后应用程序就可以处理其他逻辑了。
>     * **I/O执行者**：**操作系统内核或框架**。内核会负责整个数据的读写过程，当操作完成后，才将完成事件通知给Proactor，由Proactor分发给对应的Completion Handler。
>     * **优点**：将I/O操作完全异步化，能达到更高的并发性能，应用层逻辑也更简单，只需关心业务结果。Windows的IOCP和Linux的`io_uring`是其典型实现。
>
> **总结来说**，两者的关键分水岭在于**数据读写由谁完成**。Reactor是应用层做，Proactor是内核做。